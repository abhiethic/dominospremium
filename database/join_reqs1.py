...
class JoinReqs1:
    ...
    def __init__(self):
        from config import JOIN_REQS_DB1, FORCE_SUB_CHANNEL
        if JOIN_REQS_DB1:
            self.client = MongoClient(JOIN_REQS_DB1)
            self.pymongo_client = MongoClient(JOIN_REQS_DB1)
            self.db = self.client["JoinReqs1"]
            self.pymongo_db = self.pymongo_client["JoinReqs1"]
            self.col = self.db[str(FORCE_SUB_CHANNEL)]
            self.pymongo_col = self.pymongo_db[str(FORCE_SUB_CHANNEL)]
        else:
            self.client = None
